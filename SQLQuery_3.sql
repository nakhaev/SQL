...
AND IC.COLUMN_NAME IN ('Name', 'Description', 'Notes', 'Internal Notes', 'Synonym')
...
select DISTINCT(
'INSERT INTO @result (TableName, ColumnName, SchemaName, Exist) 
SELECT ''' + SchemaName + ''', ''' + TableName +  ''', ''' + ColumnName + 
''', (SELECT COUNT(*) FROM [BCIQ_ODS].[' + SchemaName + '].[' + TableName + '] WHERE [' + ColumnName + '] LIKE ' + '''%CHECKLY%'''+');') 
FROM @temp